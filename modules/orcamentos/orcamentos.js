  ...
  const openCreateQuoteModal = (onSuccess) => {
    ...
    const renderModal = () => {
        ...
        <div class="modal-dialog" style="max-width: 600px;">
          ...
          <div class="modal-body" style="padding: 16px; display: flex; flex-direction: column; gap: 14px;">
            ...
            <div style="background: #1e293b; padding: 12px; border-radius: 8px; border: 1px solid #334155;">
              <span style="font-size: 10px; font-weight: 700; color: #ea580c; text-transform: uppercase; display: block; margin-bottom: 8px;">Adicionar Material ao Orçamento:</span>
              <div style="display: grid; grid-template-columns: 3fr 1fr 1fr auto; gap: 8px; align-items: end;">
                <div>
                  <label style="font-size: 10px; color: #94a3b8; display: block; margin-bottom: 2px;">Material:</label>
                  <select id="select-q-prod" style="width: 100%; font-size: 11px;">
                    ${products.map(p => `<option value="${p.id}" data-price="${p.salePriceBase}" data-unit="${p.baseUnit}">[${p.code}] ${p.name} - ${p.salePriceBase} MT</option>`).join('')}
                  </select>
                </div>
                <div>
                ...
                <tbody>
                  ...
                  ` : quoteItems.map((it, idx) => `
                    <tr style="border-bottom: 1px solid #1e293b;">
                      ...
                      <td style="padding: 6px 10px;">${it.unitPrice.toFixed(2)} MT</td>
                      <td style="padding: 6px 10px; font-weight: bold; color: #34d399;">${(it.quantity * it.unitPrice).toFixed(2)} MT</td>
      ...
      if (prodSelect && unitPriceInp) {
        ...
        unitPriceInp.value = selOpt.getAttribute('data-price');
        prodSelect.onchange = () => {
  ...
  const printQuoteProposal = (quote) => {
    ...
    win.document.write(`
      ...
      <body>
          ...
          <tbody>
            ${(quote.items || []).map((it, idx) => `
              <tr>
                ...
                <td>${it.unitPrice.toFixed(2)} MT</td>
                <td>${(it.quantity * it.unitPrice).toFixed(2)} MT</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
        ...
        <div class="total">
          TOTAL GERAL DA PROPOSTA: ${quote.total.toFixed(2)} MT
        </div>
